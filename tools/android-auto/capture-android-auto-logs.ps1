...
function Resolve-DeviceSerial {
    ...
    if ($authorizedDevices.Count -eq 0) {
        throw "No authorized adb device is attached. Cannot capture Android Auto logs."
    }
...
$logDirectory = Join-Path $repoRoot "logs\android-auto"
New-Item -ItemType Directory -Force -Path $logDirectory | Out-Null

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$logFile = Join-Path $logDirectory "android-auto-$timestamp.log"
$pattern = "PtvMusic|AndroidAuto|MediaLibraryService|MediaSession|ExoPlayer|MusicPlayback|MusicRepository"
...
if ($DurationSeconds -gt 0) {
    $rawFile = Join-Path $logDirectory "android-auto-$timestamp.raw.log"
    $process = Start-Process -FilePath $resolvedAdbPath `
        -ArgumentList @($adbDeviceArgs + "logcat") `
        -NoNewWindow `
        -PassThru `
        -RedirectStandardOutput $rawFile `
        -RedirectStandardError (Join-Path $logDirectory "android-auto-$timestamp.err.log")
    Start-Sleep -Seconds $DurationSeconds
    if (-not $process.HasExited) {
        Stop-Process -Id $process.Id -Force
    }
    $filteredLines = Select-String -Path $rawFile -Pattern $pattern | ForEach-Object { $_.Line }
    if ($filteredLines) {
        $filteredLines | Set-Content -Path $logFile
    } else {
        Set-Content -Path $logFile -Value ""
    }
    Write-Host "Captured filtered logs for $DurationSeconds seconds."
    Write-Host "Filtered log: $logFile"
    Write-Host "Raw log: $rawFile"
} else {
    Write-Host "Capturing live filtered logs. Press Ctrl+C to stop."
