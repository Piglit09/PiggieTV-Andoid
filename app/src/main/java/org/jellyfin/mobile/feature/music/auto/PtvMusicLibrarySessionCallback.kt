...
internal class PtvMusicLibrarySessionCallback(
            ...
            when {
                parentId == PtvMusicAutoIds.ROOT -> {
                    val rootItems = PTV_MUSIC_AUTO_ROOT_ITEMS.map { item -> item.toAutoMediaItem() }
                        .drop(startIndex)
                        .take(limit)
                    Timber.i("PTV Music Auto root children requested count=${rootItems.size} page=$page pageSize=$pageSize")
                    rootItems
                }
    ...
    private suspend fun startGeneratedQueueMediaId(
        ...
        val firstItem = queue.firstOrNull()
            ?: throw UnsupportedOperationException("${mediaId.generatedQueueDisplayName()} has no playable songs right now.")
    ...
    private fun generatedQueueResult(
        ...
        resumeStore.markBrowseStatus("generated:$source:${mix?.name ?: mediaId.generatedQueueDisplayName()}", returnedTracks.size)
    ...
    private fun cachedMixCandidates(mix: PtvMusicAutoMix): List<MusicItem> {
        ...
        return when (mix) {
            ...
            PtvMusicAutoMix.CURRENT_ARTIST -> {
                ...
                if (artistIds.isEmpty()) emptyList() else (activeQueue + songs + recent + favorites).filter { item ->
            ...
            PtvMusicAutoMix.CURRENT_GENRE -> {
                ...
                if (genreName.isBlank()) emptyList() else (activeQueue + songs + recent + favorites).filter { item ->
    ...
    private fun activeQueueCandidates(): List<MusicItem> =
        playbackController.state.value.queue.filterPlayableDistinct()
    ...
    private fun String.generatedQueueDisplayName(): String =
        when {
...
private data class AutoGeneratedQueueResult(
