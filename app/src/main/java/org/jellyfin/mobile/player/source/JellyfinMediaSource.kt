...
sealed class JellyfinMediaSource(
    ...
    val isAudioOnly: Boolean
        get() = selectedVideoStream == null && audioStreams.isNotEmpty()
        ...
        for (mediaStream in mediaStreams) {
            when (mediaStream.type) {
                ...
                MediaStreamType.AUDIO -> {
                    audio += mediaStream
                    if (mediaStream.index ==
                        (playbackDetails?.audioStreamIndex ?: sourceInfo.defaultAudioStreamIndex)
                    ) {
                ...
                MediaStreamType.SUBTITLE -> {
                    ...
                    if (mediaStream.index ==
                        (playbackDetails?.subtitleStreamIndex ?: sourceInfo.defaultSubtitleStreamIndex)
                    ) {
    ...
    fun getName(context: Context): String = item?.let {
        buildString {
            val name = if (
