...
class PlayerFragment :
    ...
    fun onPressSpeedUp(isPressing: Boolean): Boolean =
        viewModel.setPressSpeedUp(isPressing, Constants.HOLD_SPEEDUP_MULTIPLIER)

    fun onDecoderSelected(type: DecoderType) {
        viewModel.updateDecoderType(type)
    }

    fun onReportMedia(anchor: View) {
        val reasons = MediaReportReason.entries
        val menu = PopupMenu(requireContext(), anchor)
        reasons.forEachIndexed { index, reason ->
            menu.menu.add(0, index, index, reason.displayName)
        }

        menu.setOnMenuItemClickListener { item ->
            val reason = reasons[item.itemId]
            if (reason.requiresDetails) {
                showCustomReportDialog(reason)
            } else {
                submitMediaReport(reason, null)
            }
            true
        }
    ...
    private fun showCustomReportDialog(reason: MediaReportReason) {
        ...
        AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("Send") { _, _ ->
                submitMediaReport(reason, input.text?.toString())
            }
