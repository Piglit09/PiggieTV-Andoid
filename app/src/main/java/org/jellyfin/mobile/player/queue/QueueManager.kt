...
class QueueManager(private val viewModel: PlayerViewModel,) : KoinComponent {
    ...
    private fun prepareStreams(source: RemoteJellyfinMediaSource): MediaSource {
        val videoSource = when {
            source.isAudioOnly -> createAudioMediaSource(source)
            else -> createVideoMediaSource(source)
        }
    ...
    @CheckResult
    private fun createAudioMediaSource(source: RemoteJellyfinMediaSource): MediaSource {
        val audioUrl = universalAudioApi.getUniversalAudioStreamUrl(
            itemId = source.itemId,
            deviceId = apiClient.deviceInfo.id,
            maxStreamingBitrate = source.maxStreamingBitrate ?: DEFAULT_AUDIO_STREAMING_BITRATE,
